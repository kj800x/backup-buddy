...
pub async fn create_policy(
    ...
    match web::block(move || policy_dao.create_policy(&policy_clone)).await {
        Ok(Ok(())) => HttpResponse::NoContent()
            .append_header(("HX-Redirect", format!("/backup-buddy/policy/{}", policy.id)))
            .body(""),
        Ok(Err(e)) => {
...
pub async fn edit_policy_form(
    ...
    match web::block(move || policy_dao.get_policy(&policy_id)).await {
        Ok(Ok(Some(policy))) => {
            let content = super::web::edit_policy_form(&policy);
            HttpResponse::Ok()
                .body(layout::base_layout("Edit Policy - Backup Buddy", content).into_string())
        }
        Ok(Ok(None)) => HttpResponse::NotFound().body("Policy not found"),
        Ok(Err(e)) => {
            error!(error = %e, "Failed to get policy for edit");
            HttpResponse::InternalServerError().body("Failed to get policy")
        }
        Err(e) => {
            error!(error = ?e, "Blocking error while getting policy for edit");
            HttpResponse::InternalServerError().body("Failed to get policy")
        }
    }
}
...
pub async fn edit_policy(
    ...
    let policy_dao = state.policy_dao.clone();
    let policy_clone = policy.clone();
    match web::block(move || policy_dao.update_policy(&policy_clone)).await {
        Ok(Ok(())) => HttpResponse::NoContent()
            .append_header(("HX-Redirect", format!("/backup-buddy/policy/{}", policy.id)))
            .body(""),
        Ok(Err(e)) => {
            error!(error = %e, "Failed to update policy");
            HttpResponse::InternalServerError().body("Failed to update policy")
        }
        Err(e) => {
            error!(error = ?e, "Blocking error while updating policy");
            HttpResponse::InternalServerError().body("Failed to update policy")
        }
    }
}

pub async fn delete_policy(state: web::Data<AppState>, path: web::Path<String>) -> impl Responder {
    let policy_id = match Uuid::parse_str(&path.into_inner()) {
        Ok(id) => id,
        Err(_) => {
            return HttpResponse::BadRequest().body("Invalid policy ID");
    ...
    let policy_dao = state.policy_dao.clone();
    match web::block(move || policy_dao.delete_policy(&policy_id)).await {
        Ok(Ok(())) => {
            // After delete, redirect to homepage
            HttpResponse::NoContent()
                .append_header(("HX-Redirect", "/backup-buddy"))
                .body("")
        }
        Ok(Err(e)) => {
            error!(error = %e, "Failed to delete policy");
            HttpResponse::InternalServerError().body("Failed to delete policy")
        }
        Err(e) => {
            error!(error = ?e, "Blocking error while deleting policy");
            HttpResponse::InternalServerError().body("Failed to delete policy")
        }
    }
}
